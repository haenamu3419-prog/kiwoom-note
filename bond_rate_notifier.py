...
# 기타 시장지표: (시리즈ID, 표시이름, 단위형식)
MARKET_SERIES = [
    ("DEXKOUS", "원/달러 환율", "won"),
    ("VIXCLS",  "VIX 변동성지수", "point"),
]
...
def build_message() -> str:
    ...
    for sid, name in OIL_SERIES:
        ...
        if len(data) >= 2:
            ...
            lines.append(f"▶ {name}: ${cur:.2f}  {arrow}{abs(diff):.2f} ({pct:+.1f}%)")
        else:
            lines.append(f"▶ {name}: ${cur:.2f}")
        lines.append(f"   ({cur_date} 기준)")
    ...
    lines.append("")
    lines.append("[ 환율·변동성 ]")
    for sid, name, unit in MARKET_SERIES:
        data = fetch_latest_two(sid)
        if not data:
            lines.append(f"▶ {name}: 데이터 없음")
            continue
        ...
        if unit == "won":
            cur_str = f"{cur:,.1f}원"
            fmt = lambda d: f"{abs(d):.1f}원"
        else:  # point (VIX 등)
            cur_str = f"{cur:.2f}"
            fmt = lambda d: f"{abs(d):.2f}"
        if len(data) >= 2:
            prev = data[1][1]
            diff = cur - prev
            pct = (diff / prev * 100) if prev else 0
            arrow = "▲" if diff > 0 else ("▼" if diff < 0 else "－")
            lines.append(f"▶ {name}: {cur_str}  {arrow}{fmt(diff)} ({pct:+.1f}%)")
        else:
            lines.append(f"▶ {name}: {cur_str}")
        lines.append(f"   ({cur_date} 기준)")
